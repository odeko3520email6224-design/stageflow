...
function PresetCard({ preset, eventId, event, onDelete, isAdmin, positionTypes }) {
  ...
  return (
    <div className={`bg-card border rounded-xl overflow-hidden ${isActive ? "border-primary" : "border-border"}`}>
      ...
      {editing && (
        <div className="border-t border-border px-4 py-3 space-y-2">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <Button size="sm" className="flex-1 h-8 text-xs" disabled={!editName.trim() || updateMutation.isPending}
              onClick={() => { if (editName.trim()) updateMutation.mutate({ name: editName.trim(), description: editDesc.trim(), slot_positions: editSlots }); }}>
      ...
      {confirm?.type === 'apply' && <ConfirmDialog message={`「${preset.name}」を適用しますか？\n現在のポジションは一度リセットされます。`} confirmLabel="適用" confirmVariant="default" onConfirm={() => { applyMutation.mutate(); setConfirm(null); }} onCancel={() => setConfirm(null)} />}
      {confirm?.type === 'clear' && <ConfirmDialog message="プリセットの適用を解除しますか？" confirmLabel="解除" confirmVariant="default" onConfirm={() => { clearMutation.mutate(); setConfirm(null); }} onCancel={() => setConfirm(null)} />}
      {confirm?.type === 'delete' && <ConfirmDialog message={`「${preset.name}」を削除しますか？`} confirmLabel="削除" onConfirm={() => { onDelete(preset.id); setConfirm(null); }} onCancel={() => setConfirm(null)} />}
    </div>
  );
}
        ...
        <div className="bg-card border border-border rounded-xl p-3 mb-3">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <Button size="sm" className="flex-1 h-8 text-xs"
              ...
              onClick={() => { if (presetName.trim() && Object.values(slotSelections).flat().length > 0) createMutation.mutate({ name: presetName.trim(), description: presetDesc.trim(), slot_positions: slotSelections, positions: [] }); }}>
